...
fn main() {
    ...
    let mut edged_to_test: Vec<Tile> = Vec::new();
    for (j, _) in dp.first().unwrap().iter().enumerate() {
        edged_to_test.push((0, j, 2));
    }
    for (j, _) in dp.last().unwrap().iter().enumerate() {
        edged_to_test.push((rows, j, 0));
    }
    for (i, _) in dp.iter().map(|row| row.first().unwrap()).enumerate() {
        edged_to_test.push((i, 0, 1));
    }
    for (i, _) in dp.iter().map(|row| row.last().unwrap()).enumerate() {
        edged_to_test.push((i, cols, 3));
    }
    ...
    for edge in edged_to_test {
        ...
        result = result.max(dp.iter().flatten().filter(|val| val.0).count());
        reset_array(&mut dp);
    }
...
fn reset_array(dp: &mut Vec<Vec<(bool, Vec<u8>)>>) {
    for row in dp {
        for val in row {
            val.0 = false;
            val.1.clear();
